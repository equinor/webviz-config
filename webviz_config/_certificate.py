...
def create_key(key_path):
    ...
    with open(key_path, "wb") as fileheader:
        fileheader.write(
            key.private_bytes(
...
def create_ca(args):
    ...
    with open(ca_crt_path, "wb") as fileheader:
        fileheader.write(cert.public_bytes(serialization.Encoding.PEM))
...
def create_certificate(directory):
    ...
    with open(ca_key_path, "rb") as fileheader:
        ca_key = serialization.load_pem_private_key(
            data=fileheader.read(), password=None, backend=default_backend()
        )

    with open(ca_crt_path, "rb") as fileheader:
        ca_crt = x509.load_pem_x509_certificate(
            data=fileheader.read(), backend=default_backend()
        )
    ...
    with open(server_crt_path, "wb") as fileheader:
        fileheader.write(crt.public_bytes(encoding=serialization.Encoding.PEM))
