...
'''
    def __init__(self, app, title: str, csv_file: Path,
        ...
        self.plot_option_id = 'plot-option{}'.format(uuid4())
    ...
    def plot_option_layout(self):
        ...
        divs.append(
            html.Div(
                ...
                children=[
                    html.P('Plot type'),
                    dcc.Dropdown(
                        id='{}-plottype'.format(self.plot_option_id),
                        clearable=False,
        ...
        for key, arg in self.plot_args.items():
            divs.append(
                html.Div(
                    style=self.style_options_div,
                    id='{}-div-{}'.format(self.plot_option_id, key),
                    children=[
                        html.P(key),
                        dcc.Dropdown(
                            id='{}-{}'.format(self.plot_option_id, key),
                            clearable=False,
                            options=[{'label': i, 'value': i}
    ...
    def plot_output_callbacks(self):
        ...
        for plot_arg in self.plot_args.keys():
            outputs.append(
                Output(
                    '{}-div-{}'.format(self.plot_option_id, plot_arg),
                    'style'
                )
            )
        return outputs
    ...
    def plot_input_callbacks(self):
        ...
        inputs.append(
            Input(
                '{}-plottype'.format(self.plot_option_id),
                'value')
        )
        for plot_arg in self.plot_args.keys():
            inputs.append(
                Input(
                    '{}-{}'.format(self.plot_option_id, plot_arg),
                    'value')
            )
        return inputs
