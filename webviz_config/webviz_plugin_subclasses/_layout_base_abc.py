...
class LayoutBaseABC(abc.ABC):
    def __init__(self) -> None:
        ...
        self._plugin_get_store_func: Optional[Callable[[str], str]] = None
    ...
    def _set_plugin_get_store_func(self, func: Callable[[str], str]) -> None:
        self._plugin_get_store_func = func

    def get_store_id(self, store_id: str) -> str:
        if self._plugin_get_store_func:
            return self._plugin_get_store_func(store_id)

        raise NotImplementedError(
            "This element has not been added to any plugin yet."
        )
